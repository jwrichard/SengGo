...
class MongoDB {
    ...
    // Get documents by a query
    getHistoryMoveCount(gameId, callback) {
        var collection = this._db.collection("history");
        collection.find({gameId: gameId}).toArray(function(err, data) {
    ...
    updateGame(board, callback, collection) {
      ...
      collection.updateOne({ gameId : board.gameId }, { $set: game }, function(err, result) {
